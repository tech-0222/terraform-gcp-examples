...
| [google_secret_manager_secret.demo](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/secret_manager_secret) | resource |
| [google_secret_manager_secret.demo_second](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/secret_manager_secret) | resource |
| [google_secret_manager_secret_iam_member.ksa_accessor](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/secret_manager_secret_iam_member) | resource |
| [google_secret_manager_secret_iam_member.ksa_accessor_second](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/secret_manager_secret_iam_member) | resource |
...
| bastion\_instance\_name | Bastion VM name. | `string` | `"tf-adv-gke-smcsi-vm"` | no |
...
| bastion\_subnet\_name | Subnet name for the bastion VM. Kept separate from the GKE subnet so master\_authorized\_networks can allow only this range. | `string` | `"tf-adv-gke-smcsi-mgmt-subnet"` | no |
| cluster\_name | GKE cluster name. | `string` | `"tf-adv-gke-smcsi"` | no |
| gke\_subnet\_cidr | Primary CIDR of the GKE subnet. | `string` | `"10.40.0.0/24"` | no |
| gke\_subnet\_name | Subnet name for GKE nodes. | `string` | `"tf-adv-gke-smcsi-subnet"` | no |
| grant\_ksa\_secret\_access | Grant the KSA roles/secretmanager.secretAccessor on the secret. Set to false and re-apply to watch the CSI mount fail -- that is how this example shows the binding is what makes the mount work. | `bool` | `true` | no |
| k8s\_namespace | Kubernetes namespace of the ServiceAccount that mounts the secret. Must match k8s/01-serviceaccount.yaml -- it is part of the IAM principal string. | `string` | `"default"` | no |
| k8s\_service\_account\_name | Kubernetes ServiceAccount name granted access to the secret. Must match k8s/01-serviceaccount.yaml and the Pod's serviceAccountName. | `string` | `"app-ksa"` | no |
...
| network\_name | VPC network name. | `string` | `"tf-adv-gke-smcsi-vpc"` | no |
| node\_machine\_type | Machine type for the GKE node pool. e2-small (2 vCPU) is not enough here: the Secret Manager CSI driver's DaemonSet plus GKE's own system Pods request ~99% of its allocatable CPU, leaving application Pods Pending with "Insufficient cpu". e2-medium is the smallest type observed to work. | `string` | `"e2-medium"` | no |
...
| secret\_id | Secret Manager secret ID. Terraform creates the secret itself but never its value -- add versions with `gcloud secrets versions add` so no plaintext lands in tfstate. | `string` | `"tf-adv-smcsi-demo"` | no |
| secret\_id\_second | Second Secret Manager secret ID, used by k8s/examples/01-multi-secret-one-volume.yaml to show multiple secrets in one volume. | `string` | `"tf-adv-smcsi-demo-2"` | no |
...
| Name | Description |
| ---- | ----------- |
| add\_secret\_version\_example | Example command to put a value into the secret (run locally, outside Terraform). |
...
| ksa\_principal | IAM principal for the Kubernetes ServiceAccount. This is what holds secretAccessor -- no Google Service Account is involved. |
...
| secret\_id | Secret Manager secret ID. Add a value with `gcloud secrets versions add` -- Terraform does not manage the value. |
| secret\_id\_second | Second secret ID, used by the multi-secret example manifest. |
