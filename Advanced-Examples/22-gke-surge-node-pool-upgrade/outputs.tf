...
output "node_pool_name" {
  description = "Node pool that carries the SURGE upgrade settings."
...
output "rollback_node_pool_example" {
  description = "Command that rolls a node pool back. SURGE has no soak period, so once a node is replaced there is nothing left to roll back to -- this only helps for an upgrade that was cancelled or failed partway."
...
output "show_upgrade_settings_example" {
  description = "Command to confirm the SURGE settings landed in GKE."
