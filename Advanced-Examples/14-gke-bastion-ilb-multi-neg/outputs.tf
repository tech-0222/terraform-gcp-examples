...
output "gke_zones" {
  description = "Zones the GKE node pool spans, one node each (one NEG per zone once app-a/b/c are deployed)."
  value       = var.gke_zones
}
...
output "ilb_vip" {
  description = "Internal HTTP LB shared VIP. Only set once enable_ilb=true has been applied. curl :81/:82/:83 from the bastion to reach app-a/b/c."
  value       = try(google_compute_address.ilb_vip[0].address, null)
}
