...
#
# Regional (not zonal, unlike 11/12/13): the Internal HTTP LB backend
# services in ilb.tf attach one NEG per zone in var.gke_zones, so the demo
# needs Pods actually running in multiple zones to be meaningful.
...
resource "google_container_node_pool" "primary" {
  ...
  # node_count applies per zone in node_locations, so this creates
  # length(var.gke_zones) nodes total (one per zone), not one node overall.
  node_count     = 1
