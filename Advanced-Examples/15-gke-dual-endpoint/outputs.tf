...
output "public_endpoint" {
  description = "GKE control plane public endpoint (reachable only from admin_public_cidr)."
  value       = google_container_cluster.primary.private_cluster_config[0].public_endpoint
}

output "admin_public_cidr" {
  description = "The public IP CIDR currently authorized to reach the public endpoint."
  value       = var.admin_public_cidr
}
...
output "get_credentials_example" {
  description = "Example command, run from the bastion, to fetch kubeconfig via the PRIVATE endpoint. --internal-ip is required: without it, gcloud defaults to the public endpoint even from inside the VPC, and the bastion's NAT egress IP is not in admin_public_cidr, so the connection times out."
  value       = "gcloud container clusters get-credentials ${google_container_cluster.primary.name} --internal-ip --zone=${var.zone} --project=${var.project_id}"
}

output "get_credentials_local_example" {
  description = "Example command, run from your local machine (must match admin_public_cidr), to fetch kubeconfig via the public endpoint -- no bastion needed."
