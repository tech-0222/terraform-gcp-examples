...
| ---- | ----------- | ---- | ------- | :------: |
| admin\_public\_cidr | Public IP CIDR allowed to reach the GKE public endpoint directly, bypassing the bastion (e.g. "203.0.113.10/32"). Get yours with `curl -s https://ifconfig.me` and append /32. Required -- there is no safe default for a public source range. | `string` | n/a | yes |
...
| artifact\_registry\_repository\_id | Artifact Registry Docker repository ID. | `string` | `"tf-adv-gke-dualep-repo"` | no |
| bastion\_instance\_name | Bastion VM name. | `string` | `"tf-adv-gke-dualep-vm"` | no |
...
| bastion\_subnet\_name | Subnet name for the bastion VM. Kept separate from the GKE subnet so master\_authorized\_networks can allow only this range. | `string` | `"tf-adv-gke-dualep-mgmt-subnet"` | no |
| cluster\_name | GKE cluster name. | `string` | `"tf-adv-gke-dualep"` | no |
| gke\_subnet\_cidr | Primary CIDR of the GKE subnet. | `string` | `"10.40.0.0/24"` | no |
| gke\_subnet\_name | Subnet name for GKE nodes. | `string` | `"tf-adv-gke-dualep-subnet"` | no |
...
| network\_name | VPC network name. | `string` | `"tf-adv-gke-dualep-vpc"` | no |
...
| Name | Description |
| ---- | ----------- |
| admin\_public\_cidr | The public IP CIDR currently authorized to reach the public endpoint. |
...
| get\_credentials\_example | Example command, run from the bastion, to fetch kubeconfig via the PRIVATE endpoint. --internal-ip is required: without it, gcloud defaults to the public endpoint even from inside the VPC, and the bastion's NAT egress IP is not in admin\_public\_cidr, so the connection times out. |
| get\_credentials\_local\_example | Example command, run from your local machine (must match admin\_public\_cidr), to fetch kubeconfig via the public endpoint -- no bastion needed. |
...
| public\_endpoint | GKE control plane public endpoint (reachable only from admin\_public\_cidr). |
