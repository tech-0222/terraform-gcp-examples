...
variable "network_name" {
  ...
  type        = string
  default     = "tf-adv-gke-dualep-vpc"
}

variable "gke_subnet_name" {
  description = "Subnet name for GKE nodes."
  type        = string
  default     = "tf-adv-gke-dualep-subnet"
}
...
variable "bastion_subnet_name" {
  ...
  type        = string
  default     = "tf-adv-gke-dualep-mgmt-subnet"
}
...
variable "artifact_registry_repository_id" {
  ...
  type        = string
  default     = "tf-adv-gke-dualep-repo"
}

variable "admin_public_cidr" {
  description = "Public IP CIDR allowed to reach the GKE public endpoint directly, bypassing the bastion (e.g. \"203.0.113.10/32\"). Get yours with `curl -s https://ifconfig.me` and append /32. Required -- there is no safe default for a public source range."
  type        = string

  validation {
    condition     = can(cidrhost(var.admin_public_cidr, 0))
    error_message = "admin_public_cidr must be a valid CIDR, e.g. 203.0.113.10/32."
  }
}
