...
| artifact\_registry\_repository\_id | Artifact Registry Docker repository ID. | `string` | `"tf-adv-gke-bastion-repo"` | no |
| bastion\_instance\_name | Bastion VM name. | `string` | `"tf-adv-gke-bastion-vm"` | no |
...
| bastion\_subnet\_name | Subnet name for the bastion VM. Kept separate from the GKE subnet so master\_authorized\_networks can allow only this range. | `string` | `"tf-adv-gke-bastion-mgmt-subnet"` | no |
| cluster\_name | GKE cluster name. | `string` | `"tf-adv-gke-bastion"` | no |
