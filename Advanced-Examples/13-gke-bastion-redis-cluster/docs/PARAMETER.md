...
| [google_compute_subnetwork.redis_psc](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_subnetwork) | resource |
...
| [google_network_connectivity_service_connection_policy.redis_psc](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/network_connectivity_service_connection_policy) | resource |
...
| [google_project_service.networkconnectivity](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/project_service) | resource |
...
| [google_project_service.serviceconsumermanagement](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/project_service) | resource |
| [google_redis_cluster.cache](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/redis_cluster) | resource |
...
| artifact\_registry\_repository\_id | Artifact Registry Docker repository ID. | `string` | `"tf-adv-gke-rediscl-repo"` | no |
| bastion\_instance\_name | Bastion VM name. | `string` | `"tf-adv-gke-rediscl-vm"` | no |
...
| bastion\_subnet\_name | Subnet name for the bastion VM. Kept separate from the GKE subnet so master\_authorized\_networks can allow only this range. | `string` | `"tf-adv-gke-rediscl-mgmt-subnet"` | no |
| cluster\_name | GKE cluster name. | `string` | `"tf-adv-gke-rediscl"` | no |
| gke\_subnet\_cidr | Primary CIDR of the GKE subnet. | `string` | `"10.40.0.0/24"` | no |
| gke\_subnet\_name | Subnet name for GKE nodes. | `string` | `"tf-adv-gke-rediscl-subnet"` | no |
...
| network\_name | VPC network name. | `string` | `"tf-adv-gke-rediscl-vpc"` | no |
...
| redis\_node\_type | Redis Cluster node type. REDIS\_SHARED\_CORE\_NANO is the smallest/cheapest. | `string` | `"REDIS_SHARED_CORE_NANO"` | no |
| redis\_psc\_subnet\_cidr | CIDR for the PSC endpoint subnet used by Redis Cluster. /29 is the recommended minimum. | `string` | `"10.44.0.0/29"` | no |
| redis\_replica\_count | Replicas per shard. 0 = no HA (cheapest, used here); 1+ enables failover. | `number` | `0` | no |
| redis\_shard\_count | Number of shards in the Redis Cluster. | `number` | `1` | no |
...
| redis\_host | Redis Cluster discovery endpoint address, reachable from Pods over PSC. |
| redis\_port | Redis Cluster discovery endpoint port (6379). |
