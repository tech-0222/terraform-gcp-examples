...
output "redis_host" {
  description = "Redis Cluster discovery endpoint address, reachable from Pods over PSC."
  value       = try(google_redis_cluster.cache.discovery_endpoints[0].address, null)
}

output "redis_port" {
  description = "Redis Cluster discovery endpoint port (6379)."
  value       = try(google_redis_cluster.cache.discovery_endpoints[0].port, null)
}
