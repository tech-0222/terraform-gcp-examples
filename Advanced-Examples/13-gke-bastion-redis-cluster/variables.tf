...
variable "network_name" {
  ...
  type        = string
  default     = "tf-adv-gke-rediscl-vpc"
}

variable "gke_subnet_name" {
  description = "Subnet name for GKE nodes."
  type        = string
  default     = "tf-adv-gke-rediscl-subnet"
}
...
variable "bastion_subnet_name" {
  ...
  type        = string
  default     = "tf-adv-gke-rediscl-mgmt-subnet"
}
...
variable "artifact_registry_repository_id" {
  ...
  type        = string
  default     = "tf-adv-gke-rediscl-repo"
}

variable "redis_psc_subnet_cidr" {
  description = "CIDR for the PSC endpoint subnet used by Redis Cluster. /29 is the recommended minimum."
  type        = string
  default     = "10.44.0.0/29"
}
...
variable "redis_replica_count" {
  description = "Replicas per shard. 0 = no HA (cheapest, used here); 1+ enables failover."
  type        = number
  default     = 0
}

variable "redis_node_type" {
  description = "Redis Cluster node type. REDIS_SHARED_CORE_NANO is the smallest/cheapest."
  type        = string
