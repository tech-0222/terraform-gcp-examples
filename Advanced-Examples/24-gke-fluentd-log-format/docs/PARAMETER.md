...
| bastion\_instance\_name | Bastion VM name. | `string` | `"tf-adv-gke-log-vm"` | no |
...
| bastion\_subnet\_name | Subnet name for the bastion VM. | `string` | `"tf-adv-gke-log-mgmt-subnet"` | no |
| cluster\_name | GKE cluster name. | `string` | `"tf-adv-gke-log"` | no |
| gke\_subnet\_cidr | Primary CIDR of the GKE subnet. | `string` | `"10.40.0.0/24"` | no |
| gke\_subnet\_name | Subnet name for GKE nodes. | `string` | `"tf-adv-gke-log-subnet"` | no |
...
| network\_name | VPC network name. | `string` | `"tf-adv-gke-log-vpc"` | no |
| node\_count | Nodes in the pool. Two so the app and its Fluentd sidecar are not competing with GKE's own system Pods on a single e2-medium. | `number` | `2` | no |
...
| logging\_query\_example | Command that shows what reached Cloud Logging from this cluster's workloads. |
