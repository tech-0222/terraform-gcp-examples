...
11〜28 はGKEプライベートクラスタ＋踏み台（11）を土台に広げてきた。土台に変数を足して済むものは既存のディレクトリを使い（例：ノードあたり最大Pod数は `11` に変数を追加）、cross-project peering のように土台を流用できない構成は独立した Root Module にした（`17`）。
