...
#
# Regional across var.gke_zones (not zonal like 11/12/13): GKE creates one
# standalone NEG per zone the cluster spans, so a multi-zone cluster is what
# makes "the backend references several NEGs" -- and the recovery problem --
# realistic.
...
resource "google_container_cluster" "primary" {
  ...
  resource_labels = {
    ...
    example    = "18-gke-standalone-neg-recovery"
  }
...
resource "google_container_node_pool" "primary" {
  ...
  # node_count applies per zone, so this is one node in each of
  # var.gke_zones.
  node_count     = 1
