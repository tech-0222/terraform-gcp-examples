...
# With container-native load balancing the LB talks to Pod IPs directly, so
# Google's health-check and load-balancer probers must be able to reach the
# Pod port on the nodes. Without this the backend never becomes HEALTHY.
