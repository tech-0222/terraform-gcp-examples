...
実GCP環境（`tech-0222-tf-examples`）で `fmt / init / validate / plan / apply` を実施し、Instance が `RUNNABLE`（`POSTGRES_15` / `db-f1-micro`）であることと Database `appdb` を確認後、`terraform destroy` まで完了しています。
