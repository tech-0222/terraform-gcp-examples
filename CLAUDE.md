...
### 公開リポジトリの PR・コメントの書き方

**読み手は、このリポジトリだけを見ている外部の人。** その人に向けて、変更が何をし、なぜ要るかを書く。

| 書く | 書かない |
|---|---|
| 変更の内容と理由（このリポジトリの言葉で） | 作業の経緯、うまくいかなかった話、誤って書いた・消したという打ち明け |
| 確かめたこと（コマンドと結果） | リポジトリ外の Issue 番号・ファイル名・パス、手元の環境の名前 |
| 読み手が取るべき行動（注意点・費用） | 「非公開」「作業メモ」「セッション」「指摘された」など内部の事情 |

経緯を残したいときは、公開されない場所に書く。PR やコメントは、あとから直しても編集履歴に最初の版が残る。
